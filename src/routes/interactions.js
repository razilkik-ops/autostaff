...
export function interactionRoutes(db, config) {
  ...
  router.post("/api/orders", asyncHandler(async (req, res) => {
    ...
    if (!customerName || !phone || !delivery || quantities.size === 0) return res.status(422).json({ error: "Заполните контакты и добавьте доступные товары." });
    ...
    const order = await db.$transaction(async (tx) => {
      ...
      const saved = await tx.order.create({
        data: { number: orderNumber(), userId: req.user?.id, customerName, phone, email: email || null, delivery, comment: comment || null, total, items: { create: items } },
        include: { items: true },
      });
